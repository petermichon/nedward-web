...
function newSaveButtonComponent(fileEditor: HTMLElement): HTMLElement {
  const e = newButtonComponent()
  e.title = 'Enregistrer'
  // e.textContent = '🔾 Enregistrer'
  e.addEventListener('click', () => {
    const text = fileEditor.textContent

    const promise = fetchPostFilesContent(text)
    promise.then((res) => {
      // res.text().then((text) => {
      // ...
      // })
...
async function fetchPostFilesContent(content: string): Promise<Response> {
  console.log(content)
